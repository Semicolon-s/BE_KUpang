...
const insertOrder = (
  ...
  if (
    ...
    throw new Error("value is uncorrect format");
  insert(
    "order_list",
...
const addWarehouseHistory = (productId, warhouseState, warhouseNum) => {
  if (isNaN(productId) || isNaN(warhouseNum) || isNaN(warhouseState))
    throw new Error("value is uncorrect format");
  insert(
    "warehouse_history",
    ["productId", "warhouseState", "warhouseNum"],
    [productId, warhouseState, warhouseNum]
  );
};
...
const selectProduct = async (fields, table, whereOptions) =>
  await select(fields, table, whereOptions);
const selectOrder = async (fields, table, whereOptions) =>
  await select(fields, table, whereOptions);
const selectStock = async (fields, table, whereOptions) =>
  await select(fields, table, whereOptions);
